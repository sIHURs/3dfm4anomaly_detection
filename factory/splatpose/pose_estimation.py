...
def main_pose_estimation(cur_class, 
    ...
    if retrieval == "loftr":
        ...
        print("[INFO] Loaded pose lookup with", len(pose_by_name))
    else:
    ...
    for i in tqdm(range(len(testset))):
        ...
        if retrieval == "loftr":
            ...
            if filename not in pose_by_name:
                raise KeyError(
                    f"Pose not found for {filename}. Example keys: {list(pose_by_name.keys())}"
                )
...
# todo: refactor duplicated code with main_pose_estimation for new pipeline

def render_from_estimated_poses(cur_class, 
                         result_dir, 
                         model_dir_location, 
                         k=150, 
                         verbose=False, 
    ...
    result_dir = result_dir
    output_dir = os.path.join(model_dir_location, "output") # output_dir should be the scene folder!!!
    model_dir = output_dir if os.path.isdir(output_dir) else model_dir_location # the same scence folder??? USE model_dir
    data_dir = "MAD-Sim/" if data_dir is None else data_dir
    ...
    # todo, vggt inference, tmp use existed vggt demo script
    SCENE_DIR = model_dir
    CONF = 1.1
    SEED = 0
    MAX_POINTS = 100000
    ...
    cmd = [
        sys.executable, "-u", "demo_colmap_vggt_lowvram.py",
        "--scene_dir", SCENE_DIR,
        "--conf_thres_value", str(CONF),
        "--seed", str(SEED),
