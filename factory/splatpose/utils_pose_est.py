...
class DefectDataset(Dataset):
    def __init__(self, dataset_dir, class_name, set='train', get_mask=True, get_features=True,
        ...
        if set == "train":
            self.camera_angleX = self.camera_transforms.get("camera_angle_x", None)
            self.camera_angleY = self.camera_transforms.get("camera_angle_y", self.camera_angleX)
        else:
            self.camera_angleX = None
            self.camera_angleY = None
...
class DatasetPose(Dataset):
    def __init__(self, dataset_dir, class_name, set='train', get_features=True):
        ...
        if self.width is None:
            with PILImage.open(os.path.join(self.images[0])) as img:
                self.width, self.height = img.size
...
# todo: compuete a dataset that can handle colmap format

# ================================================================
# === Basic COLMAP loader (from your provided code)
# ================================================================
...
CAMERA_MODEL_NAMES = dict([(m.model_name, m) for m in CAMERA_MODELS])
...
def focal2fov(focal, pixels):
    """Convert focal length to field of view (FoV)."""
    return 2 * atan(pixels / (2 * focal))


def colmap_w2c_to_c2w(R_wc, t_wc):
    """Convert COLMAP world→camera to camera→world."""
    R_cw = R_wc.T
    t_cw = -R_cw @ t_wc
...
class ColmapDefectDataset(Dataset):
    """
    Dataset loader for industrial defect images with COLMAP camera poses.
        ...
        dataset_dir/
            class_name/
                ...
                test/
                    ...
                    ...
                ground_truth/
                    defect_type1/*_mask.png
                    defect_type2/*_mask.png
                    ...

    This dataset:
        - Loads images, labels, and masks like DefectDataset
        - Loads camera intrinsics & poses from COLMAP (sparse/0)
        - Converts all COLMAP poses (w2c) → (c2w)
        - Allows retrieving camera pose per image via get_pose_for_index()
    """
    ...
    def __init__(
        ...
        images_colmap = read_extrinsics_binary(os.path.join(sparse_dir, "images.bin"))

        # Assume single intrinsics shared by all views
        cam0 = next(iter(cameras.values()))
        width, height = cam0.width, cam0.height

        if cam0.model == "PINHOLE":
            fx, fy, cx, cy = cam0.params
        elif cam0.model == "SIMPLE_PINHOLE":
            f, cx, cy = cam0.params
            fx = fy = f
        else:
        ...
        self.camera_angle_x = float(focal2fov(fx, width))

        # Create mapping: image filename → c2w pose
        self.pose_by_name = {}

        for img in images_colmap.values():
            R_wc = img.qvec2rotmat()
            t_wc = img.tvec
            c2w = colmap_w2c_to_c2w(R_wc, t_wc)
            self.pose_by_name[img.name] = c2w

        # ------------------------------------------------------------
        # Load images and labels from train/test folders
        # ------------------------------------------------------------
        subclasses = sorted(os.listdir(split_dir))

        class_index = 1   # label=0 reserved for "good"
        self.class_names = ["good"]
    ...
    def _colmap_key(self, img_path):
        """
        Convert dataset image path -> COLMAP image.name key.
        Here we use only the basename: "000.png".
        If your COLMAP image.name contains subfolders,
        modify this method accordingly.
        """
        return os.path.basename(img_path)

    def get_pose_for_index(self, index: int) -> np.ndarray:
        """
        Return the camera-to-world (c2w) pose for the given image index.
        Returns None if not found.
        """
        key = self._colmap_key(self.images[index])
        return self.pose_by_name.get(key, None)

    def grab_mask(self, mask_path, index):
    ...
    def __getitem__(self, index):
        ...
        if self.split == "test" and self.get_mask:
            mask_tensor = self.grab_mask(self.masks[index], index)
        else:
