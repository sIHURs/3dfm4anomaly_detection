...
from factory.gaussian_splatting_cd.scene.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, rotmat2qvec, \
    read_extrinsics_binary, read_intrinsics_binary, read_points3D_binary, read_points3D_text
from factory.gaussian_splatting_cd.utils.graphics_utils import getWorld2View2, focal2fov, fov2focal
import numpy as np
import json
from pathlib import Path
from plyfile import PlyData, PlyElement
from factory.gaussian_splatting_cd.utils.sh_utils import SH2RGB
from factory.gaussian_splatting_cd.scene.gaussian_model import BasicPointCloud
import imageio
from datetime import datetime
from tqdm import tqdm
from factory.gaussian_splatting_cd.DepthAnythingV2.depth_anything_v2.dpt import DepthAnythingV2
from scipy.stats import entropy

from pathlib import Path
DEPTHANYTHING2_CKPT_PATH = Path(__file__).resolve().parents[1] /"DepthAnythingV2" / "depth_anything_v2_vitb.pth"
    ...
    for idx, key in enumerate(sorted(cam_extrinsics,key=lambda x:cam_extrinsics[x].name)):
        ...
        if depth_model_type is not None:
            ...
            if pcd is not None and idx in train_idx:
                ...
                if debug_output:
                    import cv2
                    from factory.gaussian_splatting_cd.render import depth_colorize_with_mask
...
def refineColmapWithIndex(path, train_index):
    """
    Keep points that appear in train cameras (even if only observed once).
    Do NOT enforce multi-view constraints.
    """
    ...
    # COLMAP stores cameras in dict keyed by image_id; you sort by name and treat that order as idx.
    ordered_keys = sorted(cam_extrinsics, key=lambda x: cam_extrinsics[x].name)

    # collect all point3D ids observed in train cameras (including single-view)
    train_ptsidxlist = []
    for tidx, key in enumerate(ordered_keys):
        if tidx in train_index:
            train_ptsidxlist.append(cam_extrinsics[key].point3D_ids)

    if len(train_ptsidxlist) == 0:
        raise ValueError("train_index selects no cameras.")

    ptsidx = np.unique(np.concatenate(train_ptsidxlist))
    # remove invalid id = -1 (your original code used [1:] which assumes -1 is first after unique; safer is explicit)
    valid_ptsidx = ptsidx[ptsidx >= 0]

    # OPTIONAL: you can keep extrinsics unchanged (recommended), or filter each camera's 2D list to train-visible points.
    # Here: keep unchanged to avoid breaking alignment; comment in if you really want to shrink per-camera correspondences.
    # for key in ordered_keys:
    #     cam_valid = np.isin(cam_extrinsics[key].point3D_ids, valid_ptsidx)
    #     cam_extrinsics[key] = cam_extrinsics[key]._replace(
    #         point3D_ids=cam_extrinsics[key].point3D_ids[cam_valid],
    #         xys=cam_extrinsics[key].xys[cam_valid],
    #     )

    # Map COLMAP point ids to rows in xyz/rgb/err:
    # Your previous code assumed: valid_totalptsidx aligns 1-to-1 with xyz (assert).
    # Keep that behavior: build valid_totalptsidx from ALL cameras but WITHOUT count>=2 restriction.
    total_ptsidxlist = [cam_extrinsics[k].point3D_ids for k in ordered_keys]
    all_ptsidx = np.unique(np.concatenate(total_ptsidxlist))
    all_ptsidx = all_ptsidx[all_ptsidx >= 0]  # remove -1
    assert len(all_ptsidx) == len(xyz), "Point id ordering assumption does not match points3D reader."

    # Select xyz/rgb/err that are in valid_ptsidx
    valid3didx = np.isin(all_ptsidx, valid_ptsidx)
    xyz_sel = xyz[valid3didx]
    rgb_sel = rgb[valid3didx]
    err_sel = err[valid3didx]

    os.makedirs(os.path.join(path, "plydummy"), exist_ok=True)
    ply_path = os.path.join(path, "plydummy", f"points3D_{int(datetime.now().timestamp())}.ply")
    storePly(ply_path, xyz_sel, rgb_sel, xyzerr=err_sel)
...
def get_all_colmap_image_indices(path):
    sparse0 = os.path.join(path, "sparse", "0")
    bin_images = os.path.join(sparse0, "images.bin")
    txt_images = os.path.join(path, "sparse_txt", "images.txt")

    if os.path.exists(bin_images):
        images = read_extrinsics_binary(bin_images)  # dict: {image_id: Image}
        n = len(images)
    elif os.path.exists(txt_images):
        images = read_extrinsics_text(txt_images)
        n = len(images)
    else:
        raise FileNotFoundError("Cannot find COLMAP images.bin or images.txt to infer number of images.")
...
def readColmapSceneInfo(
    ...
    all_idx = get_all_colmap_image_indices(path)
    train_idx = all_idx
    test_idx = []
    ...
    if eval:
        np.random.seed(seed)
        train_idx = sorted(
            np.random.choice(all_idx, size=min(kshot, len(all_idx)), replace=False)
        )
    ...
    train_cam_infos = cam_infos if not eval else [cam_infos[i] for i in train_idx]
    test_cam_infos = []
